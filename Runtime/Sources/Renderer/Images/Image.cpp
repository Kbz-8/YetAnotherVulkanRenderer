...
namespace Yavr
{
	...
	VkFormat BitsToFormat(std::uint32_t bits)
	{
		switch(bits)
		{
			...
			default:
				Error("Vulkan : unsupported image bit-depth");
	...
	void Image::Init(std::uint32_t width, std::uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, std::vector<VkMemoryPropertyFlags> properties, const char* name)
	{
		...
		if(vkCreateImage(RenderCore::Get().GetDevice().Get(), &image_info, nullptr, &m_image) != VK_SUCCESS)
			FatalError("Vulkan : failed to create an image");

		VkMemoryRequirements mem_requirements;
		vkGetImageMemoryRequirements(RenderCore::Get().GetDevice().Get(), m_image, &mem_requirements);
		
		std::optional<uint32_t> mem_type_index;
		...
		if(vkAllocateMemory(RenderCore::Get().GetDevice().Get(), &alloc_info, nullptr, &m_memory) != VK_SUCCESS)
			FatalError("Vulkan : failed to allocate memory for an image");

		vkBindImageMemory(RenderCore::Get().GetDevice().Get(), m_image, m_memory, 0);
	...
	void Image::CreateImageView(VkImageViewType type, VkImageAspectFlags aspect_flags) noexcept
	{
		...
		if(res != VK_SUCCESS)
			FatalError("Vulkan : failed to create an image view, %", VerbaliseVkResult(res));
		#ifdef YAVR_DEBUG
		else
	...
	void Image::CopyFromBuffer(GPUBuffer& buffer)
	{
	...
	void Image::CopyToBuffer(GPUBuffer& buffer)
	{
		CommandBuffer& cmd = RenderCore::Get().GetSingleTimeCmdBuffer();
		cmd.BeginRecord();
