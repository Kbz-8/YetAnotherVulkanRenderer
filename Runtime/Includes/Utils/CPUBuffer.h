#ifndef __YARV_UTILS_CPU_BUFFER__
#define __YARV_UTILS_CPU_BUFFER__
...
namespace Yavr
{
	class CPUBuffer
	{
		public:
			...
			[[nodiscard]]
			inline CPUBuffer Duplicate() const
			{
			...
			template<typename T>
			[[nodiscard]]
			inline T* GetDataAs() const { return static_cast<T*>(m_data.get()); }
			[[nodiscard]]
			inline std::uint8_t* GetData() const { return m_data.get(); }
